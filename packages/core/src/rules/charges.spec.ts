...
/**
 * Data de referência FIXA. Antes, o helper usava `new Date()` real e
 * `toISOString()` (UTC) enquanto `calculateLateCharges` compara em horário
 * LOCAL — em fuso negativo, depois das 21h o UTC já virou o dia e toda a
 * contagem deslocava em 1, quebrando 4 testes de forma intermitente.
 */
const NOW = new Date(2026, 7, 12) // 2026-08-12, horário local

/** Vencimento N dias antes de NOW, construído em horário local. */
function dueDate(daysAgoN: number): string {
  const d = new Date(NOW.getFullYear(), NOW.getMonth(), NOW.getDate() - daysAgoN)
...
describe('calculateLateCharges', () => {
  it('retorna zeros quando config é null', () => {
    const result = calculateLateCharges(null, 500, dueDate(10), NOW)
    expect(result.total).toBe(0)
    expect(result.grace_period_active).toBe(false)
  })

  it('retorna zeros quando baseAmount é zero', () => {
    const result = calculateLateCharges(fixed, 0, dueDate(10), NOW)
    expect(result.total).toBe(0)
  })

  it('retorna zeros quando a cobrança não está vencida', () => {
    const result = calculateLateCharges(fixed, 500, dueDate(0), NOW)
    expect(result.total).toBe(0)
  ...
  it('CA-017: fee fixa + juros 10 dias, sem carência', () => {
    const result = calculateLateCharges(fixed, 500, dueDate(10), NOW)
  ...
  it('CA-018: dentro do período de carência → zeros com grace_period_active=true', () => {
    const result = calculateLateCharges(withGrace, 500, dueDate(3), NOW)
  ...
  it('CA-019: carência encerrada no dia 5, 1 dia de encargos', () => {
    const result = calculateLateCharges(withGrace, 500, dueDate(6), NOW)
    expect(result.grace_period_active).toBe(false)
  ...
  it('RN-010: multa fixa não duplica com mais dias', () => {
    const r10 = calculateLateCharges(fixed, 500, dueDate(10), NOW)
    const r20 = calculateLateCharges(fixed, 500, dueDate(20), NOW)
  ...
  it('RN-012: encargos calculados sobre o baseAmount informado', () => {
    ...
    const result = calculateLateCharges(fixed, base, dueDate(10), NOW)
  ...
  it('multa percentual calculada sobre baseAmount', () => {
    const result = calculateLateCharges(pct, 1000, dueDate(5), NOW)
  ...
  it('encargos zerados quando ambas as taxas são 0', () => {
    ...
    const result = calculateLateCharges(zeroConfig, 500, dueDate(10), NOW)
    expect(result.fee).toBe(0)
    expect(result.interest).toBe(0)
    expect(result.total).toBe(0)
  })
