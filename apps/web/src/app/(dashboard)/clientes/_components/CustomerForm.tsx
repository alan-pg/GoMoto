...
const selectCls = 'w-full h-9 px-3 rounded-lg bg-[#282828] border border-[#474747] text-[13px] text-[#f5f5f5] outline-none focus:border-[#BAFF1A] transition-all'
const textareaCls = 'w-full px-3 py-2 rounded-lg bg-[#282828] border border-[#474747] text-[13px] text-[#f5f5f5] placeholder:text-[#616161] outline-none focus:border-[#BAFF1A] transition-all resize-none'
...
function DocumentUpload({
  ...
  return (
    <div>
      <label className={labelCls}>{label}</label>
      {hint && <p className="text-[12px] text-[#616161] mb-2">{hint}</p>}

      {preview ? (
        <div className="relative group rounded-xl overflow-hidden border border-[#474747] bg-[#282828]">
          {pendingFile?.type === 'application/pdf' || existingUrl?.endsWith('.pdf') ? (
            <div className="flex items-center gap-3 p-4">
              <div className="w-10 h-10 rounded-lg bg-[#323232] flex items-center justify-center text-[#9e9e9e] text-[11px] font-bold">PDF</div>
              <span className="text-[13px] text-[#f5f5f5] truncate">{pendingFile?.name ?? 'documento.pdf'}</span>
            </div>
          ) : (
          ...
          <button
            type="button"
            onClick={onClear}
            className="absolute top-2 right-2 w-7 h-7 rounded-full bg-[#7c1c1c] text-[#ff9c9a] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-[#9c2c2c]"
            title="Remover"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <label className="flex flex-col items-center justify-center gap-2 h-32 rounded-xl border-2 border-dashed border-[#474747] bg-[#282828] cursor-pointer hover:border-[#BAFF1A] hover:bg-[#2f2f2f] transition-all">
          <Upload className="w-5 h-5 text-[#616161]" />
          <span className="text-[13px] text-[#616161]">Clique para selecionar</span>
          <span className="text-[11px] text-[#474747]">JPG, PNG, WebP ou PDF — máx. 10MB</span>
          <input
            type="file"
            className="sr-only"
    ...
    <div className="min-h-screen bg-[#121212]">
      ...
      <div className="flex max-w-5xl mx-auto">
        ...
        <div className="flex-1 py-6 pr-6 space-y-10 min-w-0">
          ...
          {/* ── Tipo de pessoa ───────────────────────────────────────────────── */}
          <section id="sec-type" className="bg-[#1a1a1a] rounded-2xl p-6">
                ...
                <button
                  ...
                  className={`flex-1 h-11 rounded-xl border text-[13px] font-medium transition-all ${
                    form.person_type === val
                      ? 'bg-[#BAFF1A]/10 border-[#BAFF1A] text-[#BAFF1A]'
                      : 'bg-[#282828] border-[#474747] text-[#9e9e9e] hover:border-[#616161]'
                  }`}
                >
                  {label}
                </button>
          ...
          <section id="sec-personal" className="bg-[#1a1a1a] rounded-2xl p-6">
          ...
          {!isCompany && (
            <section id="sec-license" className="bg-[#1a1a1a] rounded-2xl p-6">
                ...
                <Field label="Categoria">
                  <select className={selectCls} value={form.drivers_license_category}
                    ...
                    {CNH_CATEGORY_OPTIONS.map((cat) => (
                      <option key={cat} value={cat} className="bg-[#202020]">{cat}</option>
                    ))}
                  </select>
                </Field>
          ...
          {/* ── Documentos ──────────────────────────────────────────────────── */}
          <section id="sec-documents" className="bg-[#1a1a1a] rounded-2xl p-6">
          ...
          {/* ── Observações ──────────────────────────────────────────────────── */}
          <section id="sec-notes" className="bg-[#1a1a1a] rounded-2xl p-6">
          ...
          {isEditing && (
            <section id="sec-departure" className="bg-[#1a1a1a] rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {form.active && (
                  <p className="col-span-2 text-[12px] text-[#616161]">
