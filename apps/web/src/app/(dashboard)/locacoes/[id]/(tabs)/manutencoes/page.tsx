  ...
  if (maintenances.length === 0) {
    return (
      <div className="flex items-center justify-center rounded-xl bg-surface py-10">
        <p className="text-[13px] text-fg-mute">Nenhuma manutenção registrada no período desta locação.</p>
      </div>
    )
  }
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {maintenances.map((m) => {
              ...
              return (
                <tr key={m.id} className="h-9 border-b border-border last:border-0 hover:bg-surface-2">
                  <td className="px-4 text-fg">{m.description}</td>
                  <td className="px-4 text-fg-mute">{MAINTENANCE_TYPE_LABEL[m.type] ?? '—'}</td>
                  <td className="px-4 text-fg-soft">{fmt(m.completed_date ?? m.scheduled_date)}</td>
                  <td className="px-4 text-right font-mono text-fg">
