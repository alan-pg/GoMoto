      ...
      <div className="space-y-1.5">
        {inspection.answers.map((a) => (
          <div key={a.item_id} className="flex items-center justify-between text-[12px]">
            <span className="text-fg-mute truncate pr-2">{a.name}</span>
            <span className={`flex-shrink-0 font-semibold ${a.status === 'ok' ? 'text-success' : 'text-danger'}`}>
      ...
      {inspection.photos.length > 0 && (
        <div className="grid grid-cols-3 gap-2 pt-1">
          {inspection.photos.map((p) => (
            <div key={p.item_id} className="aspect-square rounded-lg bg-surface overflow-hidden">
              {urls[p.storage_path] && (
