-- Caução: operador passa a informar a data relevante ao criar a locação —
-- data do pagamento (se já paga) ou data de vencimento (se ainda pendente) —
-- em vez do sistema assumir now()/start_date.

DROP FUNCTION IF EXISTS create_rental_with_charges(
  UUID, UUID, UUID, TEXT, INTEGER, NUMERIC, DATE, DATE, BOOLEAN, JSONB, NUMERIC, JSONB, BOOLEAN
);
