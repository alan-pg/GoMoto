...
-- create_rental_with_charges passa a aceitar e persistir a Entrada como
-- 1 cobrança billing_type='down_payment' — mesmo padrão da Caução (paga na
-- hora ou pendente), mas sem ledger em `deposits` (RN-001: não é devolvível).
-- ============================================================
...
BEGIN
  ...
  -- Entrada (Spec 0010): sem ledger próprio — é só uma cobrança, paga na
  -- hora ou pendente. Não reembolsável, então não passa por `deposits`.
